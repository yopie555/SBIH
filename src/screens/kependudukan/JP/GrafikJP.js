...
const GrafikPP = (props) => {
  ...
  const dataPresentaseLaki = dataJumlahPenduduk.map(item => item.laki)
  const dataPresentasePerempuan = dataJumlahPenduduk.map(item => item.perempuan)
  return (
    <View style={{flex: 1 }}>
  ...
  <LineChart
    data={{
      labels: dataJumlahPenduduk && dataJumlahPenduduk.map(item => item.tahun),
      datasets: [
        {
