...
const GrafikJRTLH = (props) => {
  ...
  const getJRTLHCategory = (jumlah) => {
    ...
    if (value === 0) return { label: 'Sangat Baik', color: '#43a047' };
    if (value >= 1 && value <= 100) return { label: 'Baik', color: '#1e88e5' };
    if (value >= 101 && value <= 500) return { label: 'Perlu Perhatian', color: '#fb8c00' };
    return { label: 'Kritis', color: '#e53935' };
  };

  const currentCategory = getJRTLHCategory(latestValue);
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.periodCard}>
          <Icon name="time-outline" size={24} color="#f57c00" />
        ...
        <View style={styles.chartCard}>
          <View style={styles.chartHeader}>
            <Icon name="bar-chart" size={24} color="#f57c00" />
          ...
          <View style={styles.currentValueContainer}>
            ...
            <View style={styles.currentValueWrapper}>
              <Text style={styles.currentValueText}>
                JRTLH Terkini: <Text style={[styles.currentValue, { color: currentCategory.color }]}>{formatNumber(latestValue)} Unit</Text>
              </Text>
        ...
        <View style={styles.trendCard}>
          <View style={styles.trendHeader}>
            <Icon name="analytics-outline" size={24} color="#f57c00" />
          ...
          <View style={styles.trendContent}>
            <View style={styles.trendItem}>
              ...
              <Text style={[styles.trendValue, { 
                ...
                {latestValue < values[0] ? '↓' : '↑'} {Math.abs(latestValue - values[0])} unit
              </Text>
            </View>
            <Text style={styles.trendDescription}>
              {latestValue < values[0] 
                ? 'Tren menurun, kondisi perumahan membaik' 
                : latestValue > values[0]
                ? 'Tren meningkat, perlu program perbaikan rumah'
                : 'Stabil, perlu pemantauan berkelanjutan'}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.infoCard}>
          ...
          <View style={styles.infoContent}>
            ...
            <View style={styles.infoRow}>
              <View style={styles.infoDot} />
              <Text style={styles.infoText}>JRTLH = rumah yang tidak memenuhi syarat layak huni</Text>
            </View>
            <View style={styles.infoRow}>
              <View style={styles.infoDot} />
            ...
            <View style={styles.infoRow}>
              <View style={styles.infoDot} />
              <Text style={styles.infoText}>Semakin rendah nilai, semakin baik kondisi perumahan</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.legendCard}>
          <Text style={styles.legendTitle}>Kategori JRTLH:</Text>
          <View style={styles.legendContent}>
            <View style={styles.legendItem}>
              <View style={[styles.legendDot, { backgroundColor: '#43a047' }]} />
              <Text style={styles.legendText}>Sangat Baik (0 unit)</Text>
            </View>
            <View style={styles.legendItem}>
              <View style={[styles.legendDot, { backgroundColor: '#1e88e5' }]} />
              <Text style={styles.legendText}>Baik (1-100 unit)</Text>
            </View>
            <View style={styles.legendItem}>
              <View style={[styles.legendDot, { backgroundColor: '#fb8c00' }]} />
              <Text style={styles.legendText}>Perlu Perhatian (101-500 unit)</Text>
            </View>
            <View style={styles.legendItem}>
              <View style={[styles.legendDot, { backgroundColor: '#e53935' }]} />
              <Text style={styles.legendText}>{`Kritis (>500 unit)`}</Text>
            </View>
          </View>
        </View>
      </ScrollView>
