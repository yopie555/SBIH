...
const GrafikAHH = (props) => {
  ...
  const last5Years = dataAngkaHarapanHidup.slice(-5);
  
  // Hitung statistik dari 5 tahun terakhir
  const values = last5Years.map(item => parseFloat(item.ahh));
  ...
  const getAHHCategory = (ahh) => {
    ...
    if (value >= 75) return { label: 'Sangat Tinggi', color: '#43a047' };
    if (value >= 70 && value < 75) return { label: 'Tinggi', color: '#1e88e5' };
    if (value >= 65 && value < 70) return { label: 'Sedang', color: '#fb8c00' };
    return { label: 'Rendah', color: '#e53935' };
  };

  const currentCategory = getAHHCategory(latestValue);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.headerTop}>
          <Icon name="analytics" size={32} color="#d32f2f" />
      ...
      >
        ...
        <View style={styles.chartCard}>
          <View style={styles.chartHeader}>
            <Icon name="bar-chart" size={24} color="#d32f2f" />
          ...
          <View style={styles.currentValueContainer}>
            ...
            <View style={styles.currentValueWrapper}>
              <Text style={styles.currentValueText}>
                AHH Terkini: <Text style={[styles.currentValue, { color: currentCategory.color }]}>{latestValue} Tahun</Text>
              </Text>
        ...
        <View style={styles.infoCard}>
          <View style={styles.infoHeader}>
            <Icon name="information-circle" size={24} color="#d32f2f" />
          ...
          <View style={styles.infoContent}>
            ...
            <View style={styles.infoRow}>
              <View style={styles.infoDot} />
              <Text style={styles.infoText}>AHH = Angka Harapan Hidup (dalam tahun)</Text>
            </View>
            <View style={styles.infoRow}>
              <View style={styles.infoDot} />
        ...
        <View style={styles.legendCard}>
          <Text style={styles.legendTitle}>Kategori AHH:</Text>
          <View style={styles.legendContent}>
            <View style={styles.legendItem}>
              <View style={[styles.legendDot, { backgroundColor: '#43a047' }]} />
              <Text style={styles.legendText}>Sangat Tinggi (≥75 tahun)</Text>
            </View>
            <View style={styles.legendItem}>
              <View style={[styles.legendDot, { backgroundColor: '#1e88e5' }]} />
              <Text style={styles.legendText}>Tinggi (70-74.99 tahun)</Text>
            </View>
            <View style={styles.legendItem}>
              <View style={[styles.legendDot, { backgroundColor: '#fb8c00' }]} />
              <Text style={styles.legendText}>Sedang (65-69.99 tahun)</Text>
            </View>
            <View style={styles.legendItem}>
              <View style={[styles.legendDot, { backgroundColor: '#e53935' }]} />
              <Text style={styles.legendText}>{`Rendah (<65 tahun)`}</Text>
            </View>
          </View>
        </View>
      </ScrollView>
