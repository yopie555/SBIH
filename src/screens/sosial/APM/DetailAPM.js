...
const DetailAPM = (props) => {
  ...
  const [dataFiltered, setDataFiltered] = React.useState(dataAngkaPartisipasiMurni.filter(item => item.no === 1))
        ...
        <List.Section>
              <List.Accordion
                ...
                <List.Item title="SD"
                  ...
                  onPress={() => {setDataFiltered(dataAngkaPartisipasiMurni.filter(item => item.no === 1)),setTitleList('SD')}}
                />
                <List.Item title="SMP" 
                  //ketika onpress di klik maka akan menampilkan data SMP 13-15 TAHUN ke dalam list
                  onPress={() => {setDataFiltered(dataAngkaPartisipasiMurni.filter(item => item.no === 2)),setTitleList('SMP')}}
                />
                <List.Item title="SMA" 
                  //ketika onpress di klik maka akan menampilkan data SMA 16-18 TAHUN ke dalam list
                  onPress={() => {setDataFiltered(dataAngkaPartisipasiMurni.filter(item => item.no === 3)),setTitleList('SMA')}}
                />
              </List.Accordion>
            </List.Section>
          ...
          dataFiltered?.map((item, index) => {
            return (
              <View key={index} style={{ padding: 10, backgroundColor: '#c4c4c4', width: '90%', marginHorizontal: '5%', borderRadius: 10, marginVertical: 5 }}>
                ...
                <Text style={{ color: color.black }}>Poin : {item.apm} </Text>
